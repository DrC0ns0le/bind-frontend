...
function Breadcrumb({ items, location, isExpanded, onToggle, isMobile }) {
  ...
  return (
    <nav
      className={`flex items-center gap-2 text-sm mb-4 h-6 text-secondary transition-smooth ${
    ...
    >
      ...
      {isMobile && !isExpanded && activeItem && (
        <button
          onClick={onToggle}
          className="flex items-center text-primary text-shadow-nav active:scale-95 font-bold"
        ...
        <React.Fragment key={index}>
          ...
          {item.path ? (
            <button
              ...
              className="hover:text-primary transition-smooth uppercase cursor-pointer"
            >
              {item.label}
            </button>
          ) : (
            <span className="text-primary font-medium uppercase transition-smooth">
              {item.label}
            </span>
          )}
        </React.Fragment>
